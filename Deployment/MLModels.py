...
def TrainModel(dataFrame, TrainOnColumns, typeOfModel, testSize = 0.2):
    ...
    match typeOfModel:
        case 'XGBoost':
            ...
            model = BayesSearchCV(pipe, searchSpace, cv=2, n_iter=5, scoring='roc_auc_ovr', random_state=1, refit=True)
        ...
        case 'Random Forest':
            ...
            model = BayesSearchCV(estimator=RFModel, search_spaces=searchSpace, n_iter=5, cv=2, random_state=1)
            model.fit(xTrain, yTrain)
        ...
        case 'Neural Network':
            model = MLPClassifier(solver='lbfgs', alpha=1e-5,
                    hidden_layer_sizes=(5, 3), random_state=1, max_iter = 100)
            model.fit(xTrain, yTrain)
    return model